...
def test_raw_sql_tables_are_hidden_from_autogenerate():
    """
    The database cache and session backends create their tables with raw SQL, so
    they are absent from Base.metadata -- and autogenerate drops what it cannot
    see. A project using either backend got a migration deleting its live store.
    """
    from buraq.core.db import tables_migrations_ignore

    ignored = tables_migrations_ignore()

    assert "buraq_cache_table" in ignored
    assert "buraq_sessions" in ignored
