...
```python
from buraq.utils.translation import gettext as _, gettext_lazy as _l, ngettext, pgettext

# Immediate translation — use in views
message = _("Welcome to Buraq")

# Lazy translation — use in models, forms, class attributes
# (evaluated at request time, not at import time)
...
```html
{# Pass translated strings from the view context #}
<h1>{{ title }}</h1>
<p>{{ welcome }}</p>

{# Or use a translation helper registered in your Jinja2 environment #}
<h1>{{ _("Dashboard") }}</h1>
```
...
To serve different languages at different URL prefixes:
...
urlpatterns = [
    path("/ar", include("myapp.urls")),   # Arabic
    path("/fr", include("myapp.urls")),   # French
    path("/",   include("myapp.urls")),   # Default (English)
]
```
