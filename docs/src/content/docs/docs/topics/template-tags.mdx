...
Buraq registers 37 filters and 10 globals automatically — no `{% load %}` needed.
...
:::note[Coming from Django]
Every one of Django's built-in filters has an answer here — most from Jinja
under the same name, the rest supplied by Buraq. Django's *tags* are mostly
Jinja's own syntax: `{% for %}`, `{% if %}`, `{% block %}`, `{% extends %}`,
`{% include %}`, `{% with %}` and `{% filter %}` all work as written, `{% comment %}`
is `{# … #}`, and `{% verbatim %}` is `{% raw %}`.

Two have nothing to do here and are deliberately absent: `{% load %}`, because
Jinja has no tag libraries to load, and `{% templatetag %}`, because there is no
`{%` to escape.

The rest — `firstof`, `widthratio`, `querystring`, `cycle`, `regroup`,
`ifchanged`, `now`, `spaceless`, `lorem` (as Jinja's `lipsum`) — are globals,
called rather than written as tags: `{{ firstof(a, b) }}` instead of
`{% firstof a b %}`.
:::
