...
Buraq adds to Jinja rather than wrapping it, so the whole language is available
and its documentation is the reference for it:

- **[Template Designer Documentation](https://jinja.palletsprojects.com/en/stable/templates/)**
  — the syntax, and every built-in filter and test in detail.

Jinja's own filters are there under the names you would expect, alongside
Buraq's:
...
`groupby`, `selectattr`, `map` and `batch` have no Django equivalent and are
worth knowing — `{{ posts|selectattr("published")|list }}` replaces a filter
written in Python.

The pages here cover what Buraq adds: [its own filters and
globals](/docs/topics/template-tags), template loading, and the settings that
...
The syntax is close enough to be misleading. These are the differences that
actually bite:
...
instead — see [configuring Jinja](#configuring-jinja-itself).
