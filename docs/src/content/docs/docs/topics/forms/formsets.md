---
title: "Formsets"
description: "A formset manages a collection of identical forms — useful for editing multiple model instances in one submission."
---
...
async def manage_books(request):
    if request.method == "POST":
        ...
        formset = BookFormSet()
    return await render(request, "books/manage.html", {"formset": formset})
```
...
async def edit_articles(request):
    ...
    return await render(request, "articles/edit.html", {"formset": formset})
```
...
async def edit_post(request, pk: int):
    ...
    return await render(request, "posts/edit.html", {"post": post, "formset": formset})
```
