...
|---|---|
| `DATABASES = {"default": {...}}` | `DATABASE_URL = "postgresql+asyncpg://..."` — one database only, see [What's Different](#whats-different) |
...
- **A single database URL** replaces Django's `DATABASES` dict — and with it the
  multi-database features: no routers, no `using()`, no read replica
